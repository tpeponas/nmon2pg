...
class NMON_Import:
    ...
    def __init__(self,skip,only,pg_dbhost="127.0.0.1",pg_dbname="nmon",pg_dbuser="nmon",pg_dbpass="nmon"):
        self.skip=skip
        self.only=only
        self.lines_pattern=['^AAA','^BBBP','^DISK.+|^VG.+|^PAGING|^WLM|^NET|^NPIV|^SEA|^IOADAPT','^LPAR|^CPU_ALL|^MEM|^MEMNEW|^MEMUSE|^PAGE|^FILE|^PROC|^SUMMARY|^PCPU_ALL|^SCPU_ALL','^TOP','^ZZZZ','^UARG','^PROCAIO']
        self.lines_proc=[self.proc_info,self.proc_BBBP,self.proc_label_value,self.proc_metrics,self.proc_top,self.proc_zzzz,self.proc_uarg,self.proc_skip]
        self.host=""
        self.serial=""
        self.debug=0
        self.copydata=dict()
