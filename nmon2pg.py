...
class NMON_Import:
    ...
    def __init__(self,skip,pg_dbhost="127.0.0.1",pg_dbname="nmon",pg_dbuser="nmon",pg_dbpass="nmon"):
        self.skip=skip
        self.lines_pattern=['^AAA','^DISK.+|^VG.+|^PAGING|^WLM|^NET','^LPAR|^CPU_ALL|^MEM|^MEMNEW|^MEMUSE|^PAGE|^FILE|^PROC','^TOP','^ZZZZ','^UARG']
        self.lines_proc=[self.proc_info,self.proc_label_value,self.proc_metrics,self.proc_top,self.proc_zzzz,self.proc_uarg]
        self.host=""
        self.serial=""
        self.debug=0
        self.copydata=dict()
    ...
    def flush(self):
        for key in self.copydata.keys():
            ...
            try:
                print("Import "+key+" for "+host);
                cur.copy_from(self.copydata[key],key,sep=',',null='#')
                self.db_conn.commit()
                cur.close()
            except:
                self.db_conn.rollback()
                print ("Caught Data error during copy "+key+ " for "+ host)
            cur.close()
